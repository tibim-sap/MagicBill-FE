...
@Component({
  selector: 'app-factura-storno',
  templateUrl: './factura-storno.component.html',
  styleUrls: ['./factura-storno.component.scss']
})
export class FacturaStornoComponent implements OnInit {
  ...
  public facturaStornoForm: FormGroup = new FormGroup({
  ...
  public onSubmit(){
    const { gestiune, denumireProdusServiciu, um, cantitate, pretFaraTva, valoare, valTva} = this.facturaStornoForm.value

    console.log(this.facturaStornoForm);
    const produs = {
  ...
  public exportPdf() {
    ...
    doc.save('factura-storno_' + Date() + '.pdf')
  }
